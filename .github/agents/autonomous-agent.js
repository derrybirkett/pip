...
/**
 * Check for existing automated PRs
 */
function hasExistingAutomatedPRs() {
  try {
    const prs = JSON.parse(
      exec('gh pr list --label automated --label roadmap --state open --json number,title --limit 50', { silent: true })
    );
    return prs.length > 0;
  } catch (error) {
    // If command fails (e.g., no PRs), return false
    return false;
  }
}
...
function hasExistingPRForIssue(issueNumber) {
  try {
    // Check for PRs that mention the issue number in the body or title
    const prs = JSON.parse(
      exec(`gh pr list --state open --json number,title,body --limit 50`, { silent: true })
    );
    ...
    const hasPR = prs.some(pr => {
      ...
      return body.includes(issueRef) || body.includes(`Fixes ${issueRef}`) || body.includes(`Closes ${issueRef}`);
    });
